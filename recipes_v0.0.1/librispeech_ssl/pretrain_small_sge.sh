...
EXPERIMENT="bestrq_sge_small_input_norm_v2.31112"
...
if [ "$N_GPUS" -gt 1 ]; then
  torchrun --standalone --nnodes=1 --nproc-per-node=$N_GPUS src/trainers/pretrain.py "${args[@]}"
else
  python src/trainers/pretrain.py "${args[@]}"
fi
