...
class CTCRescorerLogitsProcessor(LogitsProcessor):
    def __init__(
        ...
        encoder_att_mask = torch.arange(encoder_logits.shape[1], device=encoder_logits.device).unsqueeze(0) > encoder_output_lens.unsqueeze(1)
    ...
    def analyze_predictions(
        ...
        def print_prediction(best_ids, name):
            ...
            new_tensor[:, 1::2] = 1
            print(f"{name}:")
            for index, (next_ids, scores) in enumerate(zip(self.tokenizer.batch_decode(new_tensor), best_ids.values)):
                print(f"HYP {index}:\n{next_ids} {scores}")

        print(f"PREFIX:")
        for index, prefix in enumerate(self.tokenizer.batch_decode(input_ids)):
            print(f"HYP {index}:\n{prefix}")
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        ids_to_score = torch.topk(scores, k=500).indices
        decoded_len = (input_ids != -1).sum(dim=-1) -1
        to_be_decoded = input_ids[:, -1] != self.eos_token_id

        ctc_scores_local, ctc_states_local = self.ctc_prefix_scorer(input_ids, ids_to_score[to_be_decoded],
                                                                    decoded_len, to_be_decoded,
