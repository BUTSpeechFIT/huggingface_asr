...
class HubertModelWrapper(HubertModel):
...
if __name__ == "__main__":
    ...
    if "whisper" in model_args.base_encoder_model:
        ...
        raise NotImplementedError("only [Whisper, WavLm, Wav2Vec2] are supported")
    ...
    if model_args.from_pretrained:
        ...
        model = TMPSpeechEncoderConnectorLMDecoder.from_pretrained(
            model_path, config, encoder, decoder, not conn_args.decoder_lora, tokenizer
        )
        ...
        model = TMPSpeechEncoderConnectorLMDecoder(
            encoder=encoder,
            decoder=decoder,
            config=apmo_config,
    ...
    if gen_args.no_metrics:
        ...
        c_metrics = lambda pred: compute_metrics_spokenwoz(
            tokenizer,
            pred,
